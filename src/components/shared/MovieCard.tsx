...
export default function MovieCard({ movie }: MovieCardProps) {
    ...
    const openTelegramModal = useTelegramModalStore((state) => state.openModal);
    const [imageError, setImageError] = useState(false);
    const [isHovered, setIsHovered] = useState(false);

    // Handle both movies and TV shows
    const isTV = !!(movie as any).name;
    const isTelegram = !!(movie as any).is_telegram;
    ...
    return (
        <motion.div
            ...
            onClick={() => isTelegram ? openTelegramModal({
                id: (movie as any).message_id || movie.id,
                title: (movie as any).title || '',
                channel_id: (movie as any).channel_id || '',
                message_id: (movie as any).message_id || movie.id,
                overview: (movie as any).overview || '',
                size: (movie as any).raw_size || 0,
                mime_type: (movie as any).mime_type || 'video/mp4',
                is_telegram: true
            }) : openModal(movie)}
            onKeyDown={(e) => e.key === 'Enter' && (isTelegram ? openTelegramModal({
                id: (movie as any).message_id || movie.id,
                title: (movie as any).title || '',
                channel_id: (movie as any).channel_id || '',
                message_id: (movie as any).message_id || movie.id,
        ...
        >
            {/* Movie Poster */}
            {isTelegram ? (
                <div className="h-full w-full bg-gradient-to-br from-blue-900/40 via-[#1c1c1e] to-black flex flex-col items-center justify-center p-4 border border-blue-500/20">
                    <Send className="w-10 h-10 text-blue-500 mb-4 opacity-70" />
                    <span className="text-gray-200 font-bold text-sm tracking-wide text-center line-clamp-4 leading-relaxed">{title}</span>
                </div>
            ) : !imageError ? (
                <img
                    src={posterUrl}
            ...
            <div className={`absolute inset-0 bg-gradient-to-t ${isTelegram ? 'from-[#0a0a1a] via-blue-900/30' : 'from-black via-black/40'} to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300`} />
            ...
            >
                {isTelegram ? (
                    <>
                        {/* Telegram hover content */}
                        <div className="flex items-center gap-2 mb-3">
                            <button
                                onClick={(e) => { e.stopPropagation(); }}
                                className="bg-blue-500/90 backdrop-blur-sm rounded-full p-2.5 hover:bg-blue-400 transition transform hover:scale-105 active:scale-95 shadow-lg shadow-blue-500/30"
                            >
                        ...
                        <h3 className="text-white font-semibold text-sm line-clamp-2 mb-1.5 tracking-tight leading-snug">
                            {title?.replace(/\.[^/.]+$/, '').replace(/[@_]/g, ' ').replace(/\s+/g, ' ').trim()}
                        </h3>

                        <div className="flex items-center gap-2 text-xs mb-2 font-medium flex-wrap">
                            <span className="text-blue-400 font-bold flex items-center gap-1">
                                <Send className="w-3 h-3" /> Telegram
                            </span>
                            <span className="text-gray-400">
                                {(movie as any).raw_size > 1024 * 1024 * 1024 
                                    ? `${((movie as any).raw_size / 1024 / 1024 / 1024).toFixed(1)} GB` 
                                    : `${((movie as any).raw_size / 1024 / 1024).toFixed(0)} MB`}
                            </span>
                            <span className="border border-blue-500/30 px-1.5 py-0.5 rounded text-[10px] uppercase text-blue-300 bg-blue-500/10">
                                {title?.split('.').pop()?.toUpperCase() || 'VIDEO'}
                            </span>
                        </div>
                    </>
                ) : (
