...
export default function Top10Card({ movie, ranking }: Top10CardProps) {
    ...
    return (
        ...
        >
            {/* Ranking Number */}
            <div className="absolute left-[-10%] md:left-[-15%] bottom-[-5%] z-10 h-full w-[60%] flex items-end justify-center pointer-events-none">
                <svg
                    viewBox="0 0 100 150"
            ...
            <div className="relative h-full w-[60%] ml-auto z-0">
                ...
                <div 
                    className="absolute inset-0 bg-gradient-to-t from-black via-black/60 to-transparent opacity-0 group-hover:opacity-100 rounded-md flex flex-col justify-between p-3 hidden md:flex"
                ...
                >
                    ...
                    <div className="flex justify-center">
                        {isContentReleased(releaseDate) ? (
                            <div 
                                className="bg-white/20 backdrop-blur-sm rounded-full p-2 transform translate-y-[-20px] group-hover:translate-y-0 hover:bg-white/40 active:scale-[0.97]"
                                style={{ transition: 'transform 200ms var(--ease-out), background-color 200ms var(--ease-out)' }}
                            >
                                <Play className="h-5 w-5 fill-white text-white" />
                            </div>
                        ) : (
                            <div 
                                className="bg-gray-600/40 backdrop-blur-sm rounded-full p-2 transform translate-y-[-20px] group-hover:translate-y-0"
